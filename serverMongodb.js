...
app.get('/check/:period', async (req, res) => {
    ...
    try {
        ...
        if (period === '0') {
            const results = await client.db("checkname").collection("check_mor_even").find({}).sort({ Id: 1 }).toArray();
            res.json(results);
        } else {
            const results = await client.db("checkname").collection("check_mor_even").distinct("Evening");
            res.json(results);
        }
    } catch (error) {
...
app.put('/updateEvening', async (req, res) => {
    const input = req.body;
    const client = new MongoClient(uri);
